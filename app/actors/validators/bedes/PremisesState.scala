...
object PremisesState extends BedesValidatorCompanion {
  ...
  def props(guid: String,
    ...
    Props(new PremisesState(guid, name, propertyId, validatorCategory, arguments))
}
...
/**
  ...
  */
case class PremisesState(guid: String,
                                  name: String,
                                  propertyId: String,
                                  validatorCategory: Option[String],
  ...
  def isValid(refId: UUID, value: Option[Seq[BEDESTransformResult]]): Future[Validator.MapValid] = {
    sourceValidateFromComponents(value).map { results =>
      if (results.exists(_.valid == false)) {
        Validator.MapValid(valid = false, Option("Missing State"))
      } else {
